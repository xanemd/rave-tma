...
function parseUrl(rawUrl) {
  ...
  // VK — загружаем как iframe (не через yt-dlp)
  const vkHosts = ['vk.com', 'm.vk.com', 'vkvideo.ru', 'vk.cc'];
  if (
    vkHosts.includes(host) ||
    host.endsWith('.vk.com') ||
    host.endsWith('.vkvideo.ru')
  ) {
    // Пытаемся построить embed URL для vkvideo.ru
    const embedUrl = buildEmbedUrl(parsed);
    if (embedUrl) {
      return { type: SOURCE_TYPES.IFRAME, payload: { embedUrl, originalUrl: url } };
    }
    // Если не удалось распарсить ID — загружаем как iframe напрямую
